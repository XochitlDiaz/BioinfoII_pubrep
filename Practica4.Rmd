...
Git hub project: <https://github.com/XochitlDiaz/BioinfoII>
...
Data was obtained through a repository from an experiment for expression
profiling by high throughput sequencing of RUNX1.

RUNX1 is a transcription factor that can function both as an oncogene
and tumor suppressor in breast cancer. Because RUNX1 alters chromatin
structure, the original experiment was performed to characterize
genome-wide RUNX1-mediated transcription and genome organization, but
here we only analyze RUNX1-mediated transcription.

Experiments where conducted with MCF-7 (breast cancer) cell line. For
the control, 3 cell lines where treated with shNS (short hairpin
Non-Specific RNA). For the knock down of RUNX1, 3 cell lines where
treated with shRUNX1.

We expect that RUNX1 depletion leads to up-regulation of genes
associated with chromatin structure and down-regulation of genes related
to extracellular matrix biology.

We will perform an differential expression analysis of the 6 single-end
reads in order to test the previous hypothesis.
